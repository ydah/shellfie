  ...
  class Config
    module Validatable
      ...
      def validate_window_theme!
        return if @window_theme.nil? || Themes::Registry.valid_window_theme?(@window_theme)

        raise ValidationError, "Invalid window_theme '#{@window_theme}'"
      end

      def validate_color_scheme!
        return if Themes::Registry.valid_color_scheme?(@color_scheme)
      ...
      def validate_appearance!
        ...
        raise ValidationError, 'colors values must be strings' unless @colors.values.all?(String)
        ...
        return unless shadow.key?(:color) && !shadow[:color].is_a?(String)
      ...
      def validate_background_gradient!
        ...
        return if gradient.is_a?(Array) && gradient.size == 2 && gradient.all?(String)
