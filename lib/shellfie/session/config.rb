  ...
  module Session
    class Config
      ...
      def initialize(raw, path: nil, source_paths: nil)
        ...
        @terminal = defaults.merge(symbolize_keys(raw[:terminal] || {}))
        ...
        @outputs = Array(raw[:outputs]).map { |output| symbolize_keys(output) }
        @render = symbolize_keys(raw[:render] || {})
        %i[window font animation].each do |key|
          @render[key] = symbolize_keys(@render[key]) if @render[key].is_a?(Hash)
        end
      ...
      def interpolate_variables(value, variables)
        ...
        when String
          if (match = /\A\{\{([A-Za-z_][A-Za-z0-9_]*)\}\}\z/.match(value))
            return fetch_variable(match[1], variables)
          end

          value.gsub(/\{\{([A-Za-z_][A-Za-z0-9_]*)\}\}/) { fetch_variable(Regexp.last_match(1), variables).to_s }
        else
          value
        end
      end

      def fetch_variable(name, variables)
      ...
      def condition_matches?(value)
        condition = symbolize_keys(value)
      ...
      def validate_terminal
        ...
        raise ValidationError, 'terminal.timeout must be positive' unless parse_duration(terminal[:timeout]).positive?

        parse_duration(terminal[:total_timeout]) unless terminal[:total_timeout].nil?
      end
      ...
      def validate_step(step, index)
        action = fetch_step_action(step, index)
      ...
      def fetch_step_action(step, index)
      ...
      def validate_step_options(step, action, value, index)
        parse_duration(step[:timeout]) if step.key?(:timeout)
        parse_duration(step[:delay]) if step.key?(:delay)
        parse_duration(value) if action == :sleep
      ...
      def validate_wait(value, index)
        ...
        parse_duration(condition[:stable]) if condition.key?(:stable)
        parse_duration(condition[:timeout]) if condition.key?(:timeout)
      ...
      def validate_expect(value, index)
        ...
        %i[elapsed_under elapsed_over].each { |key| parse_duration(condition[key]) if condition.key?(key) }
      end
