    ...
    class SVGRenderer
      ...
      def line_svg(line, geometry, top)
        ...
        segments = line[:segments].map do |segment|
          width = Terminal::TextMetrics.pixel_width(
      ...
      def segment_svg(segment, x, top, width, geometry)
        ...
        [
          segment_background(background, x, top, width, geometry),
          segment_text(segment, foreground, x, top, geometry)
        ].compact.join("\n")
      end
      ...
      def segment_colors(segment)
        ...
        return [foreground, background] unless segment.reverse

        [background || theme.colors[:background], foreground]
      end

      def segment_background(color, x, top, width, geometry)
        return unless color

        %(<rect x="#{x}" y="#{top}" width="#{width}" height="#{geometry[:scaled_line_height]}" fill="#{escape(color)}"/>)
      end

      def segment_text(segment, foreground, x, top, geometry)
        attributes = [font_attributes(geometry[:font_config], geometry[:scaled_font_size], segment)]
        attributes << %(fill="#{escape(foreground)}")
        attributes.concat(segment_text_attributes(segment))
        text = %(<text x="#{x}" y="#{top + geometry[:scaled_font_size]}" xml:space="preserve" #{attributes.join(' ')}>#{escape(segment.text)}</text>)
        segment.link ? %(<a href="#{escape(segment.link)}">#{text}</a>) : text
      end

      def segment_text_attributes(segment)
        attributes = []
        attributes << %(fill-opacity="0") if segment.conceal
        attributes << %(fill-opacity="0.6") if segment.dim && !segment.conceal
        attributes << %(class="blink") if segment.blink
        decorations = segment_decorations(segment)
        attributes << %(text-decoration="#{decorations.join(' ')}") unless decorations.empty?

        underline_style = segment.underline_style == :curly ? :wavy : segment.underline_style
        attributes << %(text-decoration-style="#{underline_style}") if underline_style && underline_style != :single
        if segment.underline_color
          attributes << %(text-decoration-color="#{escape(theme.color_for(segment.underline_color))}")
        end
        attributes
      end

      def segment_decorations(segment)
        decorations = []
        decorations << 'underline' if segment.underline
        decorations << 'line-through' if segment.strikethrough
        decorations << 'overline' if segment.overline
        decorations
      end
