    ...
    private
      ...
      def wrap_line(line, max_cells)
        ...
        line[:segments].each do |segment|
          Terminal::TextMetrics.graphemes(segment.text).each do |char|
            width = Terminal::TextMetrics.grapheme_width(char, ambiguous_width: ambiguous_width)
      ...
      def clip_segments(segments, max_cells)
        ...
        segments.each_with_object([]) do |segment, result|
          ...
          text = Terminal::TextMetrics.take_cells(segment.text, remaining, ambiguous_width: ambiguous_width)
          next if text.empty?

          result << copy_segment(segment, text)
          remaining -= Terminal::TextMetrics.cell_width(text, ambiguous_width: ambiguous_width)
        end
      end

      def scroll_segments(segments, max_cells)
        total_cells = segments.sum { |segment| Terminal::TextMetrics.cell_width(segment.text, ambiguous_width: ambiguous_width) }
        ...
        segments.each do |segment|
          segment_cells = Terminal::TextMetrics.cell_width(segment.text, ambiguous_width: ambiguous_width)
          ...
          text = if cells_to_drop.positive?
                   Terminal::TextMetrics.drop_cells(segment.text, cells_to_drop,
