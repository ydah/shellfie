  ...
  module CLI::Generate
    ...
    def run_generate
      ...
      jobs = input_files.map do |input_file|
        ...
        format = if @options[:default_output]
                   @options[:format] || (animate ? 'gif' : 'png')
                 else
                   output_format_for(
                     @options[:output], animate
                   )
                 end
    ...
    def build_generate_parser
      OptionParser.new do |opts|
        ...
        opts.on('--preset NAME', 'readme, ogp, widescreen, standard, or vertical') do |name|
          unless ASPECT_PRESETS.key?(name)
            raise ValidationError,
                  "preset must be one of: #{ASPECT_PRESETS.keys.join(', ')}"
          end
        ...
        opts.on('--format FORMAT',
                'Output format (png, gif, svg, svg-raster, webp, apng, mp4, webm, png-sequence, html, txt, ansi, json, asciicast)') do |format|
    ...
    def write_rendered_output(config, output_path, animate:, format:, announce: true)
      ...
      result = if SEMANTIC_FORMATS.include?(format)
                 TranscriptRenderer.new(config).render(output_path, format: format,
    ...
    def parse_framerate(value)
      ...
      return fps if fps&.between?(1, 120)
    ...
    def output_path_for(input_file, format, multiple:, config:)
      ...
      return File.join(File.dirname(input_file), "#{name}.#{format}") if @options[:default_output]
