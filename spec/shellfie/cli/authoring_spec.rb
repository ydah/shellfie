...
RSpec.describe Shellfie::CLI do
  ...
  it 'prints schemas and shell completions' do
    expect { described_class.new(%w[schema 2]).run }.to output(/Shellfie executable session/).to_stdout
    expect { described_class.new(%w[completion bash]).run }.to output(/complete.*shellfie/).to_stdout
    expect { described_class.new(%w[completion powershell]).run }.to output(/Register-ArgumentCompleter/).to_stdout
  end
  ...
  it 'runs version 2 sessions when an included file changes' do
    Dir.mktmpdir do |dir|
      ...
      cli = described_class.new(['watch', input, '-o', File.join(dir, 'out.svg'), '--interval', '0.01'])
      generator = instance_double(described_class, run: nil)
      expect(described_class).to receive(:new).with(['run', input, '-o', File.join(dir, 'out.svg'),
                                                     '--force']).and_return(generator)
