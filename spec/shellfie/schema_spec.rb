...
RSpec.describe 'configuration schemas' do
  ...
  it 'uses valid schemas that accept the shipped examples' do
    ...
    (Dir['examples/*.yml'] - ['examples/session.yml']).each do |path|
      expect(v1.valid?(YAML.safe_load_file(path, aliases: true))).to be(true), path
    end
    expect(v2.valid?(YAML.safe_load_file('examples/session.yml', aliases: true))).to be true
  end
  ...
  it 'rejects structures rejected by runtime validation' do
    ...
    expect(v1.valid?({ 'window' => { 'width' => 120, 'padding' => 100 },
                       'lines' => [{ 'output' => 'x' }] })).to be false
    ...
    expect(v2.valid?({ 'version' => 2, 'steps' => [],
                       'render' => { 'window' => { 'width' => 120, 'padding' => 100 } } })).to be false
    expect(v2.valid?({ 'version' => 2, 'steps' => [],
                       'render' => { 'window' => { 'width' => 200, 'padding' => 41 } } })).to be false
    expect(v2.valid?({ 'version' => 2, 'steps' => [],
                       'render' => { 'animation' => { 'final_delay' => 86_400_001 } } })).to be false
    ...
    expect(v2.valid?({ 'version' => 2,
                       'steps' => [{ 'run' => 'x', 'async' => true, 'visibility' => 'hidden' }] })).to be false
    expect(v2.valid?({ 'version' => 2, 'steps' => Array.new(101) do |index|
      { 'capture' => "c#{index}" }
    end })).to be false
    expect(v2.valid?({ 'version' => 2, 'steps' => [],
                       'outputs' => [{ 'path' => 'x', 'format' => 'PNG' }] })).to be false
  end
