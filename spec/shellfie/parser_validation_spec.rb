...
RSpec.describe Shellfie::Parser do
  describe '.parse_string validation' do
    ...
    it 'wraps invalid YAML aliases as ParseError' do
      expect do
        described_class.parse_string("lines: *missing\n")
      end.to raise_error(Shellfie::ParseError, /unknown (?:alias|anchor)/i)
    end
    ...
    it 'raises ValidationError for unknown top-level keys' do
      ...
      expect do
        described_class.parse_string(yaml)
      end.to raise_error(Shellfie::ValidationError, /Unknown configuration key/)
    end
