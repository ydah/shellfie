...
RSpec.describe Shellfie::Config do
  describe '#initialize' do
    ...
    it 'validates window dimensions' do
      ...
      expect do
        described_class.new(window: { padding: -1 })
      end.to raise_error(Shellfie::ValidationError, /window.padding/)
      expect do
        described_class.new(window: { width: 200, padding: 41 })
      end.to raise_error(Shellfie::ValidationError, /padding/)
    ...
    it 'validates opacity and overflow mode' do
      expect do
        described_class.new(window: { opacity: 1.5 })
      end.to raise_error(Shellfie::ValidationError, /window.opacity/)
      expect do
        described_class.new(window: { scroll_offset: 1.1 })
      end.to raise_error(Shellfie::ValidationError, /scroll_offset/)
      expect do
        described_class.new(window: { overflow: 'spill' })
      end.to raise_error(Shellfie::ValidationError, /window.overflow/)
    end
    ...
    it 'validates boolean settings' do
      ...
      expect do
        described_class.new(animation: { loop: 'yes' })
      end.to raise_error(Shellfie::ValidationError, /animation.loop/)
    ...
    it 'validates animation settings' do
      expect do
        described_class.new(animation: { typing_chunk_size: 0 })
      end.to raise_error(Shellfie::ValidationError,
                         /typing_chunk_size/)
      expect do
        described_class.new(animation: { typing_jitter: 2.0 })
      end.to raise_error(Shellfie::ValidationError, /typing_jitter/)
      expect do
        described_class.new(animation: { palette: 'bad' })
      end.to raise_error(Shellfie::ValidationError, /animation.palette/)
      expect do
        described_class.new(animation: { scroll_easing: 'bad' })
      end.to raise_error(Shellfie::ValidationError, /scroll_easing/)
      expect do
        described_class.new(animation: { gif_colors: 1 })
      end.to raise_error(Shellfie::ValidationError, /gif_colors/)
      expect do
        described_class.new(animation: { webp_quality: 101 })
      end.to raise_error(Shellfie::ValidationError, /webp_quality/)
      expect do
        described_class.new(animation: { webp_method: 7 })
      end.to raise_error(Shellfie::ValidationError, /webp_method/)
      expect do
        described_class.new(animation: { loop_count: 65_536 })
      end.to raise_error(Shellfie::ValidationError, /loop_count/)
      expect do
        described_class.new(animation: { apng_prediction: 'magic' })
      end.to raise_error(Shellfie::ValidationError,
                         /apng_prediction/)
    end
    ...
    it 'enforces resource limits' do
      expect do
        described_class.new(limits: { max_lines: 1 },
                            lines: [Shellfie::Line.new(output: '1'),
                                    Shellfie::Line.new(output: '2')])
